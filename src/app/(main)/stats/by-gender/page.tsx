...
async function getRecords() {
  ...
  function sortResultsByAgeAndScore(results: any) {
    const resultsByAge = results.reduce((acc: any, result: any) => {
      const age = result.assesment.gender;
      if (!acc[age]) {
        acc[age] = [];
      }
      acc[age].push(result);
  ...
  const report: { id: number; name: string | null; genders: {} }[] = [];
  competences.forEach((competence) => {
    report.push({
      id: competence.id,
      name: competence.name,
      genders: sortResultsByAgeAndScore(competence.results),
    });
  });
                    ...
                    <table className="min-w-full divide-y divide-gray-300">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          {records[0].genders.map((gender: any) => (
                            <>
                              <th
                                scope="col"
                                className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900"
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {records.map((competence) => (
                          <tr key={competence.id}>
                            ...
                            {competence.genders.map((gender: any) => (
                              <>
                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                  {gender.minScore}
                                </td>
