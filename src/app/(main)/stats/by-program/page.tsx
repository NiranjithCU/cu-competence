...
async function getRecords() {
  ...
  function sortResultsByAgeAndScore(results: any) {
    const resultsByAge = results.reduce((acc: any, result: any) => {
      const age = result.assesment.program;
      if (!acc[age]) {
        acc[age] = [];
      }
      acc[age].push(result);
  ...
  const report: { id: number; name: string | null; programs: {} }[] = [];
  competences.forEach((competence) => {
    report.push({
      id: competence.id,
      name: competence.name,
      programs: sortResultsByAgeAndScore(competence.results),
    });
  });
                    ...
                    <table className="min-w-full divide-y divide-gray-300">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          {records[0].programs.map((program: any) => (
                            <>
                              <th
                                scope="col"
                                className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900"
                              ...
                              <th
                                ...
                                className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900"
                              >
                                Mean ({program.program})
                              </th>
                              <th
                                scope="col"
                                className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900"
                              >
                                Median ({program.program})
                              </th>
                              <th
                                scope="col"
                                className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900"
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {records.map((competence) => (
                          <tr key={competence.id}>
                            ...
                            {competence.programs.map((program: any) => (
                              <>
                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                  {program.minScore}
                                </td>
